...
// Firestore's `where` is case-sensitive, so this normalizes to lowercase
// on both write (registerForEvent) and read here, to keep the lookup
// resilient to how the visitor capitalizes their email.
export async function getRegistrationsByEmail(email) {
  if (!email) return [];
  const normalized = email.trim().toLowerCase();
  const q = query(collection(db, "registrations"), where("emailLower", "==", normalized));
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
...
export async function registerForEvent(eventId, { name, email, role }) {
  ...
  if (!dupeSnap.empty) throw new Error("You're already registered for this event");
